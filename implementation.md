...
**Version:** 0.1.0 | **Date:** 2026-05-05
**Package version:** 0.1.0 (see `package.json` — bumps on feature milestones, independent of doc version)
...
| 192 | Diagnostic report viewer (mobile commits in flight) | 🚧 | Phase 192 backend Commit 1 landed (`5a12195` — videos section variant 5 + migration 040 + atomic worker + F9 int-typed heuristic refinement). Mobile Commit 2 (this commit) adds `useReport(sessionId)` hook + `ReportDocument`/`ReportSection` discriminated-union types + 5 type-guards (`isRowsSection` / `isBulletsSection` / `isTableSection` / `isBodySection` / `isVideosSection`) for the viewer to discriminate section variants without unsafe casts. 27 new mobile tests (9 hook + 18 type-guard); 320/320 mobile suite green. Mirrors `useSession` shape (`{report, isLoading, error, refetch}`). F29 ADR posture preserved at the route level (cross-owner returns 404, free-tier can read own report); hook surfaces backend responses unchanged. Pre-migration-040 NULL `analyzing_started_at` handling left for Commit 3's stuck-detection logic (Contract A). Mobile package 0.0.9→0.1.0; mobile impl.md 0.0.11→0.1.0. |
