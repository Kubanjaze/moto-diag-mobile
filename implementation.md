...
**Version:** 0.2.6 | **Date:** 2026-09-07 (Phase 204 — Gate 10: film → diagnose → share verified on device; Track I closes)
...
| 204 | Gate 10 — mobile integration test + TestFlight | ✅ | `204_*.md`. **Track I closes.** Headline flow verified on a physical iPhone end to end. Five bugs found by running it: F46 (permission deadlock — `not-determined` collapses to `'unknown'`, whose branch rendered a spinner with no way to reach `request()`; PhotoCaptureScreen was never stuck because it renders a Grant button, which is why photo/voice smoked and video never did), ffmpeg 9 removing `-vsync`, `width`/`height` as `0` slipping through `??`, discarded 422 reasons, and one self-inflicted handler regression. TestFlight `.ipa` production-signed — Phase 199 had shipped ONE entitlements file for both configs, so a tester build would have requested a sandbox push token and failed silently. **F64 filed as a release blocker** (share-link base URL). |
