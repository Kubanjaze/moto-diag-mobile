...
**Version:** 0.1.1 | **Date:** 2026-05-05
**Package version:** 0.1.1 (see `package.json` — bumps on feature milestones, independent of doc version)
...
| 192 | Diagnostic report viewer (mobile commits in flight) | 🚧 | Phase 192 backend Commit 1 landed (`5a12195` — videos section variant 5 + migration 040 + atomic worker + F9 int-typed heuristic refinement). Mobile Commit 2 added `useReport(sessionId)` hook + `ReportDocument`/`ReportSection` discriminated-union types + 5 type-guards. Mobile Commit 3 (this commit) adds `ReportViewerScreen` + `SectionToggle` (preset selector chip row) + `ReportSectionCard` (discriminated-union renderer for all 5 section variants including videos with nested findings + state chips + stuck advisories). Pure-logic helpers extracted for testability: `reportPresets.ts` (full/customer/insurance presets + per-section override map per Section C1 (γ) data shape), `reportStuckDetection.ts` (Contract A consumer: NULL `analyzing_started_at` → stuck immediately; non-NULL with elapsed > 5min → stuck-timeout; <= 5min → in-flight; defensive malformed-ISO handling), `reportFormatters.ts` (duration / bytes / locale-timestamp / video meta-line). 43 new mobile tests across 3 new pure-logic test files (16 preset + 13 stuck-detection + 14 formatter); 363/363 mobile suite green. Per-section override-map data shape exists from day one (F28 deferred for per-card UI). `SessionsStackParamList.ReportViewer` route entry added; navigator registration + SessionDetail cross-link deferred to Commit 4. F30 candidate filed informally for live-tick refresh (currently re-evaluates stuck-state on focus + preset change only). Mobile package 0.1.0→0.1.1; mobile impl.md 0.1.0→0.1.1. |
| 193-204 | (remaining Track I) | 🔲 | (will land in backend `completed/` as they ship) |
