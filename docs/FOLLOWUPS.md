...
### F20 (NEW) — Generalize Phase 191C's no-hardcoded-model-ids lint to "no hardcoded SSOT-managed constants in tests"

- **Surfaced:** Phase 191B fix-cycle-5 (2026-05-04). Backend full regression after Phase 191C 5b finalize surfaced 2 pre-existing failures both stemming from the same root cause: tests pinning a literal value of an SSOT-managed constant. Specifically `tests/test_phase184_gate9.py::TestGate9AntiRegression::test_schema_version_unchanged` asserted `SCHEMA_VERSION == 38` (literal) instead of importing the canonical value or using a `KNOWN_GOOD_SCHEMA_VERSIONS` set. When Phase 191B's migration 039 bumped `motodiag.core.database.SCHEMA_VERSION` to 39 the test stayed pinned at 38 — a Phase 191B finalize miss the team only discovered ~94 minutes into the post-191C regression sweep.
- **Severity:** architectural. Same shape as F9 subspecies (ii) "hardcoded source-of-truth values" but generalized beyond model IDs. Phase 191C's `motodiag/no-hardcoded-model-ids-in-tests` (mobile) + `scripts/check_f9_patterns.py --check-model-ids` (backend) are intentionally narrow per plan v1.0.1: regex-matches `claude-(haiku\|sonnet\|opus)-[\d\-]+` only. The same heuristic shape — "literal value in test that should reference a typed import from the SSOT module" — generalizes to: schema versions, API versions, package versions, OpenAPI schema digest hashes, migration sequence numbers, anything else with a single canonical home in production code that tests should import rather than literal-pin.
- **Scope estimate:** medium. Two pieces: (a) extend backend `scripts/check_f9_patterns.py` with a new `--check-ssot-constants` mode that takes a config file declaring `{constant_name: source_module}` pairs (e.g., `SCHEMA_VERSION: motodiag.core.database`) and flags any `tests/**/*.py` file containing a literal RHS for the named constant outside import-from-SSOT or set-membership patterns; (b) mirror with a mobile ESLint rule `motodiag/no-hardcoded-ssot-constants-in-tests` reading from `eslint-plugin-motodiag/ssot-constants.json`. Wire as `error` from day one (no warn-rollout phase) — by the time this rule lands the team has the discipline pattern from Phase 191C 5a's clean-baseline cleanup.
- **Decision:** **Recommended target Phase 192 lead ticket OR a dedicated Phase 191D** (depending on user pacing). Until landed, the discipline established at Phase 191C 5a still applies: when bumping any SSOT-managed constant in production, grep tests/ for literal pins and update them in the same commit (the 5b finalize regression sweep that caught these failures is now the first-line gate).
- **Repro:** the canonical example is the very fix in this commit — `tests/test_phase184_gate9.py:584` asserted `SCHEMA_VERSION == 38` instead of importing it from `motodiag.core.database` and asserting `SCHEMA_VERSION == 39`.

### F21 (NEW) — TAG_CATALOG should be auto-derived from route definitions (or diff-checked)

- **Surfaced:** Phase 191B fix-cycle-5 (2026-05-04). Sister failure to F20: `tests/test_phase183_openapi.py::TestTags::test_tag_catalog_covers_used_tags` failed because Phase 191B's `src/motodiag/api/routes/videos.py` declared `APIRouter(prefix="/sessions", tags=["videos"])` but Phase 183's `TAG_CATALOG` (in `src/motodiag/api/openapi.py`) was never updated to include the "videos" entry with description. Same Phase 191B finalize miss as F20.
- **Severity:** architectural. Two parallel sources of truth — route declarations carry tags, TAG_CATALOG carries tag descriptions, and they're maintained separately. Test that should have caught the drift (`test_tag_catalog_covers_used_tags`) ran clean in Phase 191B's targeted Phase 191B test sweep but wasn't part of the per-phase smoke; only the full backend regression caught it.
- **Scope estimate:** medium-large. Three options: (a) auto-derive TAG_CATALOG from route definitions via FastAPI introspection — descriptions move out of `openapi.py` and into per-router metadata (e.g., a `description` kwarg or a `TAG_DESCRIPTIONS` constant per route module); (b) keep TAG_CATALOG as the canonical source but write a pre-commit hook (or extend `scripts/check_f9_patterns.py` with a `--check-tag-catalog-coverage` mode) that scans `src/motodiag/api/routes/**/*.py` for `tags=[...]` literals and asserts each appears in TAG_CATALOG; (c) flip the assertion direction: make adding a new route with a tag-not-in-TAG_CATALOG a hard error in `app_factory.py` at import time (fail-fast vs fail-on-test-run).
- **Decision:** **Recommended target Phase 192+ alongside F20** (both are F9-family refinements + both surfaced together). Option (b) is the lowest-disruption: it lives in the same lint script as the existing checks and follows the same opt-out + diagnostic-output conventions. Option (a) is the cleanest long-term but a larger refactor than the immediate F9 intervention scope. Option (c) is too aggressive for a docs-tag concern (would break dev-time iteration on a new route before the OpenAPI doc is updated).
- **Repro:** the canonical example is the very fix in this commit — `src/motodiag/api/routes/videos.py:91` declared `tags=["videos"]` but `src/motodiag/api/openapi.py:131` did not include a "videos" entry in TAG_CATALOG.
