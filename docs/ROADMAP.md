...
| 192B | Diagnostic report PDF export + Share Sheet | 🔲 | Feature half of the substrate-then-feature pair (192 viewer / 192B PDF + Share). Mobile-side PDF download via `react-native-share` · iOS UIActivityViewController + Android ACTION_SEND wiring · backend gains sibling `POST /v1/reports/session/{id}/pdf` for preset-filtered PDFs (composer-side filter, renderer pure per Phase 192 shape doc) · GET sibling unchanged · deterministic-rendering pytest at Commit 1 (fail-fast if reportlab non-deterministic, F34 candidate) · belt-and-suspenders temp-file cleanup. 3 commits planned (1 backend + 2 mobile + 1 finalize). Plan v1.0 written 2026-05-05 with F33 process refinement applied. |
